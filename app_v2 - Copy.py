...
# ==========================================
with tab_plan:
    st.subheader("🗓️ Planning Hebdomadaire & Prévu vs Réalisé")
    ...
    if target_plan_id:
        date_ref = st.date_input("Semaine du :", value=datetime.date.today(), key="d_ref_plan")
        
        raw_seances = supabase.table("seances").select("*").eq("athlete_id", target_plan_id).execute().data
        if raw_seances:
            df_s = pd.DataFrame(raw_seances)
            df_s['date_seance'] = pd.to_datetime(df_s['date_seance'])

            dt_ref = pd.to_datetime(date_ref)
            lundi = dt_ref - pd.Timedelta(days=dt_ref.weekday())
            dimanche = lundi + pd.Timedelta(days=6)

            df_semaine = df_s[(df_s['date_seance'] >= lundi) & (df_s['date_seance'] <= dimanche)].copy()
            ...
            if not df_semaine.empty:
                ...
                st.markdown("### 📋 Tableau Hebdomadaire Détaillé")
                ...
                st.download_button(
                    label="📥 Télécharger le bilan hebdo (CSV)",
                    data=csv_data,
                    file_name=f"bilan_hebdo_{lundi.strftime('%Y%m%d')}.csv",
                    mime="text/csv"
                )
            else:
                st.info("Aucune séance enregistrée pour cette semaine.")
